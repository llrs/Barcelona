...
saveRDS(out_model, "output/model2_ileum_optimization.RDS")
out2 <- readRDS("output/model2_ileum_optimization.RDS")
...
saveRDS(model2b_sgcca, "output/model2b_ileum_sgcca.RDS")
...
saveRDS(model1.1_sgcca, "output/model1.1_ileum_sgcca.RDS")

# Estimated time of 1 hours
out_model <- search_model(A = Ab, c1 = shrinkage, scheme = "centroid",
                          scale = FALSE, verbose = FALSE,
                          bias = TRUE, nWeights = 11)
saveRDS(out_model, "output/model2_ileum_optimization_IBD.RDS")
out2 <- readRDS("output/model2_ileum_optimization_IBD.RDS")
...
saveRDS(model2b2_sgcca, "output/model2b2_ileum_sgcca.RDS")
